...
51. Which has a larger cardinality, `Tailnum` or `Year`?[^51]
    (a) `Tailnum`
    (b) `Year`
    (c) they have the same cardinality
    (d) neither has a cardinality

[^51]: a. `Tailnum`. The cardinality is the number of unique values, and there are many more unique planes than years.


---

52. Which index takes up more storage space, the one on `Tailnum` or the one on `Year`?[^52]
    (a) `Tailnum`
    (b) `Year`
    (c) they take up the same space
    (d) you can't index on either variable

[^52]: a. `Tailnum`. Because the cardinality is higher, it will take up much more space in the index.

---

53. Which index is more effective at reducing querying time, the one on `Tailnum` or the one on `Year`?[^53]
    (a) `Tailnum`
    (b) `Year`
    (c) the queries would be the same
    (d) you can't query on either variable
    
[^53]: a. `Tailnum`. Because the index is more complete, it will make the querying more efficient.

---

54. What does the **R** function `ifelse(a, b, c)` do?[^54]
...
[^54]: c. a = question, b = TRUE option, c = FALSE option

---

55. What does the **R** function `case_when()` do?[^55]
...
[^55]: d. creates a new variable by re-coding an original variable

---

56. What does the **R** function `cut()` do?[^56]
    (a) renames a variable
    (b) changes the data type of a variable
    (c) partitions a numeric variable
    (d) creates a new variable by re-coding an original variable

[^56]: c. partitions a numeric variable
