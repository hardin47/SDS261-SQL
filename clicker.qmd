...
38. Which **SQL** statement is used to create a database table called 'Customers'?[^59]
    (a) CREATE DATABASE TAB Customers
    (b) CREATE TABLE Customers
    (c) CREATE DATABASE TABLE Customers
    (d) CREATE DB Customers

[^38]: b. CREATE TABLE Customers

---

39. Which **SQL** statement revises data in a database?[^39]
    (a) SAVE AS 
    (b) MODIFY
    (c) SAVE
    (d) UPDATE
...
40. Which **SQL** statement takes out data from a database?[^40]
    (a) REMOVE
    (b) DELETE
    (c) COLLAPSE
...
41. The NOT NULL constraint enforces a column to not accept NULL values.[^41]
    (a) FALSE
    (b) TRUE
...
42. Which **SQL** statement places new data in a database?[^42]
    (a) `ADD RECORD`
    (b) `INSERT INTO`
    (c) `ADD NEW`
    (d) `INSERT NEW` 
...
43. With **SQL**, how can you insert a new record into the "Persons" table?[^43]
    (a) `INSERT` `INTO` Persons VALUES ('Jimmy', 'Jackson')
    (b) `INSERT` ('Jimmy', 'Jacskon') `INTO` Persons
    (c) `INSERT` VALUES ('Jimmy', 'Jackson') `INTO` Persons

[^43]: a. `INSERT` `INTO` Persons VALUES ('Jimmy', 'Jackson')

---

44. With **SQL**, how can you insert "Olsen" as the "LastName" in the "Persons" table?[^44]
    (a) `INSERT` `INTO` Persons (LastName) VALUES ('Olsen')
    (b) `INSERT` `INTO` Persons ('Olsen) `INTO` LastName
    (c) `INSERT` ('Olsen') `INTO` Persons (LastName)
 
[^44]: a. `INSERT` `INTO` Persons (LastName) VALUES ('Olsen')

---

45. How can you change "Hansen" into "Nilsen" in the "LastName" column in the Persons table?[^45]
...
[^45]: d. `UPDATE` Persons SET LastName='Nilsen' `WHERE` LastName='Hansen' 

---

46. With **SQL**, how can you delete the records where the "FirstName" is "Peter" in the Persons Table?[^46]
...
[^46]: a. `DELETE` `FROM` Persons `WHERE` FirstName='Peter'
