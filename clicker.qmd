...
57. In **SQL**, which operator is used to search for a specified pattern in a column?[^57]
    (a) `LIKE`
    (b) `GET`
    (c) `FROM`
...
[^58]: e. `SELECT` * `FROM` Persons `WHERE` FirstName `REGEXP` '(?i)a.*' (n.b., the `LIKE` function will give you a similar result, with `%` as a wildcard: SELECT` * `FROM` Persons `WHERE` FirstName LIKE 'a%')
