...
1. Set up a connection within an **R** chunk:
...
2. Pull in the `flights` table as a `tbl` (do *not* use `collect()`!).  Use **dplyr** commands to calculate the number of flights that flew into or out of BDL in 2012.  Then use `dplyr::get_query()` to output the **SQL** query.  (All done inside an **R** chunk.)
...
I needed to remove the back ticks from the table and variable names.  R uses the backticks, but DBeaver does not.  Good to remember.
...
10. render - commit - push to GitHub.  You should push three files, those with extensions .qmd, .html, and .sql. In a browser, look at your GitHub website to make sure that all three files were successfully pushed to the correct repo.
